  ...
  async function handleInviteMember(e: SyntheticEvent) {
    ...
    try {
      const newMember = await apiFetch<Member>(`/api/workspaces/${workspaceId}/members `,{
          method: "POST",
          body: JSON.stringify({email: newMemberName, role: Number(inviteRole)})}
      );
        ...
        {isAdmin && (
          <form onSubmit={handleInviteMember} className="mb-4 flex gap-2">
            ...
            <select
              ...
              className="rounded-lg border border-gray-200 px-3 py-2 transition-colors focus:border-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-400"
            >
